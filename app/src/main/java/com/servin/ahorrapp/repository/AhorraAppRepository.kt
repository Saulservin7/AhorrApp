...
class AhorraAppRepository @Inject constructor(private val ahorraAppDatabase: AhorraAppDatabase) {
    ...
    suspend fun deleteRoom(room: Rooms) = ahorraAppDatabase.roomsDao().deleteRoom(room)
     suspend fun updateTotalSaving(id: Int, totalSaving: Long) =
        ahorraAppDatabase.roomsDao().updateTotalSaving(id, totalSaving)
    fun getLastRoom(): Flow<Rooms> =
        ahorraAppDatabase.roomsDao().getLastRoom().flowOn(Dispatchers.IO).conflate()

    suspend fun getRoomById(id: Int): Rooms =
        ahorraAppDatabase.roomsDao().getRoomById(id).flowOn(Dispatchers.IO).first()


   /* suspend fun updateRuletaGame(id: Int, newGame: String) {
        ahorraAppDatabase.roomsDao().updateRuletaGame(id, newGame)
    }*/
