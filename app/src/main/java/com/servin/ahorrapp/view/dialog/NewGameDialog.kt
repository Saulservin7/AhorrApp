        ...
        Column(
            ...
            Row(
                ...
                TextField(
                    ...
                    onValueChange = { value ->
                        if (value.all { it.isDigit() && value.length <= 4 }) {
                            rouletteViewModel.setInitValue(value)
                        }
                    },
                    label = {
                        Text("Mín")
                ...
                TextField(
                    ...
                    onValueChange = { value ->
                        if (value.all { it.isDigit() && value.length <= 4 }) {
                            rouletteViewModel.setFinalValue(value)
                        }
                    },
                    label = { Text("Máx") }
                )
            ...
            Button(
                enabled = if (rouletteViewModel.initvalue.value.isNotEmpty() && rouletteViewModel.finalvalue.value.isNotEmpty())
                    rouletteViewModel.initvalue.value.toInt() < rouletteViewModel.finalvalue.value.toInt()
                else false,
                onClick = {
                    ...
                    val ruletaGame =
                        Game.Ruleta(rangeStart = initValue, rangeEnd = finalValue, type = "ruleta")
